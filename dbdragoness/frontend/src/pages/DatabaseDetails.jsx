...
function DatabaseDetails() {
  ...
  const [extraJoins, setExtraJoins] = useState([]); // [{id, type, table, alias, on}]
  const [nextJoinId, setNextJoinId] = useState(1);
  ...
  const handleAggregationSubmit = async (e) => {
    ...
    try {
      // Build the list of all joins (OG + extras)
      const allJoins = [];
      if (dbType === "sql" && aggJoinTable) {
        allJoins.push({
          table: aggJoinTable,
      ...
      extraJoins.forEach((j) => {
        if (j.table && j.on) {
          allJoins.push({
            table: j.table,
            table_alias: j.alias || undefined,
            on: j.on,
            type: j.type,
          });
        }
      ...
      const response = await axios.post(`/api/db/${dbName}/aggregation`, {
        ...
        having: aggHaving.trim() || undefined,
  ...
  const addExtraJoin = () => {
    setExtraJoins([
      ...extraJoins,
      { id: nextJoinId, type: "INNER", table: "", alias: "", on: "" },
    ]);
    setNextJoinId(nextJoinId + 1);
  };

  const removeExtraJoin = (id) => {
    setExtraJoins(extraJoins.filter((j) => j.id !== id));
  };

  const updateExtraJoin = (id, field, value) => {
    setExtraJoins(
      extraJoins.map((j) => (j.id === id ? { ...j, [field]: value } : j))
    );
  };
  ...
  const handleDeleteTable = async (tableName) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setTables(prev => prev.filter(t => (t.name || t) !== tableName));
        if (searchTerm) {
          performSearch(searchTerm);
        } else {
          fetchTables();
            ...
            {aggSubTab === "visual" && !supportsPipeline && (
              <form onSubmit={handleAggregationSubmit}>
                ...
                >
                  ...
                  {/* HAVING — only shown when GROUP BY is filled and DB is SQL */}
                  {dbType === "sql" && aggGroupBy.trim() && (
                    <div style={{ marginBottom: "15px" }}>
                      <label
                        style={{
                          color: "#f0f0f0",
                      ...
                      <input
                        type="text"
                        value={aggHaving}
                        onChange={(e) => setAggHaving(e.target.value)}
                        placeholder="e.g., SUM(oi.quantity * p.price) > 100"
                        style={{
                          padding: "8px",
                          width: "400px",
                          border: "1.5px solid #9370db",
                      ...
                      <div style={{ fontSize: "12px", color: "#999", marginTop: "4px" }}>
                        💡 Filter groups after aggregation — use the same expressions as your SELECT fields
                      </div>
                    </div>
                  )}
                  ...
                  {extraJoins.map((join) => (
                    ...
                    >
                      ...
                      >
                        <h4 style={{ color: "#f0f0f0", margin: 0 }}>
                          Additional JOIN
                        </h4>
                        <button
                          type="button"
                          onClick={() => removeExtraJoin(join.id)}
                          style={{
                            padding: "4px 10px",
                            background: "#dc3545",
                            color: "white",
                      ...
                      {dbType === "sql" && (
                        <div style={{ marginBottom: "10px" }}>
                          <label style={{ color: "#f0f0f0", display: "block", marginBottom: "5px" }}>
                            Join Type:
                          </label>
                          <select
                            value={join.type}
                            onChange={(e) => updateExtraJoin(join.id, "type", e.target.value)}
                            style={{
                              padding: "8px",
                              width: "200px",
                              border: "1.5px solid #9370db",
                      ...
                      <div style={{ marginBottom: "10px" }}>
                        <label style={{ color: "#f0f0f0", display: "block", marginBottom: "5px" }}>
                          {dbType === "nosql" ? "Lookup Collection:" : "Join Table:"}
                        </label>
                        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                          <select
                            value={join.table}
                            onChange={(e) => updateExtraJoin(join.id, "table", e.target.value)}
                            style={{
                              padding: "8px",
                              width: "300px",
                              border: "1.5px solid #9370db",
                          ...
                          >
                            <option value="">No {dbType === "nosql" ? "lookup" : "join"}</option>
                            {tables.map((t) => {
                              const name = typeof t === "string" ? t : t.name;
                              return (
                          ...
                          {dbType === "sql" && (
                            <>
                              ...
                              <input
                                type="text"
                                value={join.alias}
                                onChange={(e) => updateExtraJoin(join.id, "alias", e.target.value)}
                                placeholder="alias (optional)"
                                style={{
                                  padding: "8px",
                                  width: "150px",
                      ...
                      <div style={{ marginBottom: "10px" }}>
                        <label style={{ color: "#f0f0f0", display: "block", marginBottom: "5px" }}>
                          {dbType === "nosql" ? "Local Field:" : "Join ON condition:"}
                        </label>
                        <input
                          type="text"
                          value={join.on}
                          onChange={(e) => updateExtraJoin(join.id, "on", e.target.value)}
                          placeholder={
                            dbType === "nosql"
                              ? "e.g., department"
                              : "e.g., table1.id = table2.user_id"
                  ...
                  {/* Add Join button — SQL only */}
                  {dbType === "sql" && (
                    <div style={{ marginTop: "15px" }}>
                      <button
                        type="button"
                        onClick={addExtraJoin}
                        style={{
                          padding: "8px 18px",
                          background: "#17a2b8",
                          color: "white",
