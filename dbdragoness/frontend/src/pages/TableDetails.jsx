...
function TableDetails() {
  ...
  const [insertModalError, setInsertModalError] = useState("");
  ...
  const [plsqlNotices, setPlsqlNotices] = useState(() => {
    // Restore notices that were saved before a post-execution page reload
    try {
      const saved = sessionStorage.getItem("plsql_notices_pending");
      if (saved) {
        sessionStorage.removeItem("plsql_notices_pending");
        return JSON.parse(saved);
      }
    } catch (_) {}
    return [];
  });
  const [plsqlNoticesDismissed, setPlsqlNoticesDismissed] = useState(false);
  ...
  // Restore success message saved before a post-PL/SQL page reload
  useEffect(() => {
    try {
      const savedMsg = sessionStorage.getItem("plsql_success_pending");
      if (savedMsg) {
        sessionStorage.removeItem("plsql_success_pending");
        setMessage(savedMsg);
      }
    } catch (_) {}
  }, []);
  ...
  const fetchTableData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setAllRecordCount(response.data.record_count);
        setAllTotalPages(response.data.total_pages);
  ...
  const handleInsert = async (e) => {
    ...
    setError("");
    setMessage("");
    setInsertModalError(""); // Clear modal error
    ...
    for (const col of columns) {
      ...
      if (col.pk && !col.autoincrement && isEmpty) {
        const errorMsg = `${col.name} is a primary key without autoincrement - you must provide a value`;
        setError(errorMsg);
        setInsertModalError(errorMsg); // Show in modal too
        return;
      }

      // ✅ NOT NULL fields (except autoincrement) MUST have values
      if (col.notnull && !col.autoincrement && isEmpty) {
        const errorMsg = `${col.name} cannot be empty (NOT NULL constraint)`;
        setError(errorMsg);
        setInsertModalError(errorMsg); // Show in modal too
        return;
      }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const errorMsg = response.data.error || "Failed to insert row";
        setError(errorMsg);
        setInsertModalError(errorMsg); // Show in modal too
      }
    } catch (err) {
  ...
  const handleUpdate = async (e) => {
    ...
    setError("");
    setMessage("");
    setUpdateModalError(""); // Clear modal error

    if (!updateCondition.trim()) {
      const errorMsg = "Update condition (WHERE clause) is required";
      setError(errorMsg);
      setUpdateModalError(errorMsg); // Show in modal too
      return;
    }
    ...
    if (Object.keys(data).length === 0) {
      const errorMsg = "At least one column value must be provided for update";
      setError(errorMsg);
      setUpdateModalError(errorMsg); // Show in modal too
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const errorMsg = response.data.error || "Failed to update rows";
        setError(errorMsg);
        setUpdateModalError(errorMsg); // Show in modal too
      }
    } catch (err) {
      ...
      return prev.map((col, i) => {
        if (i === index) {
          return {
            ...col,
            pk: checked,
            // When checking PK: force notnull+unique on. When unchecking: clear only these two.
            notnull: checked ? true : false,
            unique: checked ? true : false,
          };
        }
        // ✅ Allow composite PKs — never touch other columns when toggling one PK
        return col;
      });
    });
  };
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (
          ...
          // Refresh structure and reload page after 2 seconds
          fetchStructure();
          fetchTableData();
          window.location.reload();
        }
  ...
  const handleQuerySubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Save query to history — don't let a history error kill the rest
        try { await addQuery(executedQuery); } catch (_) {}
        setQueryText("");
        ...
        } else if (resultType === "status" || resultType === "empty") {
          setQueryResult(null);
          if (resultType === "empty") {
            ...
            if (result.rows_affected !== undefined) {
              ...
              setMessage(result.status || result.message || "Query executed successfully");
            }
          } else {
            setMessage("Query executed successfully");
          }
        } else if (Array.isArray(result) && result.length > 0) {
          // No result_type but got rows — show them
          setQueryResult(result);
          setMessage("Query executed successfully");
        } else {
          setQueryResult(null);
        ...
        // ✅ Always soft-refresh the data tab — unconditional, wrapped safely
        try {
          const refreshResponse = await axios.get(`/api/table/${dbName}/${tableName}`, {
            params: { page, per_page: perPage, sort_column: sortColumn, sort_order: sortOrder },
          });
          if (refreshResponse.data.success) {
            setRows(refreshResponse.data.rows);
            setAllRows(refreshResponse.data.rows);
            setRecordCount(refreshResponse.data.record_count);
            setTotalPages(refreshResponse.data.total_pages);
            setAllRecordCount(refreshResponse.data.record_count);
            setAllTotalPages(refreshResponse.data.total_pages);
          }
        } catch (refreshErr) {
          console.warn("Soft refresh after query failed:", refreshErr);
        }
      } else {
  ...
  const handleExecutePlsql = async (e) => {
    ...
    setQueryResult(null);
    setPlsqlNotices([]);
    setPlsqlNoticesDismissed(false);
    ...
    try {
      ...
      console.log("[PL/SQL] Raw API response:", response.data);

      if (response.data.success) {
        const rawNotices = response.data.notices || [];
        console.log("[PL/SQL] Raw notices from server:", rawNotices);

        const notices = rawNotices.filter((n) => n && n.trim() !== "");
        console.log("[PL/SQL] Filtered notices to display:", notices);
        ...
        // Always update notices state — even empty — so the container re-renders cleanly
        console.log("[PL/SQL] Calling setPlsqlNotices with:", notices);
        setPlsqlNotices(notices);
        setPlsqlNoticesDismissed(false);
        console.log(
          "[PL/SQL] setPlsqlNotices called. notices.length =",
          notices.length,
        );

        // Soft data refresh — NO page reload
        ...
        console.log("[PL/SQL] Execution failed:", response.data.error);
        setError(response.data.error || "Failed to execute PL/SQL");
      }
    } catch (err) {
      console.error("[PL/SQL] Exception caught:", err);
      setError(
                        ...
                        <thead>
                          <tr>
                            {(() => {
                              ...
                              const resultKeys = Object.keys(queryResult[0]);

                              // Try to order them according to table schema
                              const orderedKeys =
                                columns.length > 0
                                  ? columns
                                      .map((col) => col.name)
                                      .filter((name) =>
                                        resultKeys.includes(name),
                                      )
                                      .concat(
                                        resultKeys.filter(
                                          (key) =>
                          ...
                          {queryResult.map((row, idx) => {
                            ...
                            const orderedKeys =
                              columns.length > 0
                                ? columns
                                    .map((col) => col.name)
                                    .filter((name) => resultKeys.includes(name))
                                    .concat(
                                      resultKeys.filter(
                                        (key) =>
                                          !columns.some(
          ...
          <div>
            ...
            <form onSubmit={handleExecuteProcedure}>
              <textarea
                ...
                rows={15}
                cols={120}
                placeholder={`Enter procedure/function code`}
                style={{
                  width: "100%",
                  padding: "10px",
                  border: "1.5px solid #9370db",
...
RAISE NOTICE 'Capitalized: % (id: %)', NEW.name, NEW.id;`}
        ...
        {tab === "plsql" && (
          <div>
            {/* PL/SQL Notices — permanent container, always visible on this tab */}
            <div
              style={{
                marginTop: "20px",
                padding: "15px",
            ...
            >
              ...
              {plsqlNotices.length === 0 || plsqlNoticesDismissed ? (
                <p
                  style={{
                    color: "#7a6a99",
                    fontStyle: "italic",
                ...
                <>
                  <button
                    onClick={() => {
                      console.log("[PL/SQL] Notices dismissed by user.");
                      setPlsqlNoticesDismissed(true);
                    }}
                  ...
                  {plsqlNotices.map((notice, idx) => {
                    console.log(`[PL/SQL] Rendering notice [${idx}]:`, notice);
                    return (
                      <div
                        key={idx}
                        style={{
              ...
              <div style={{ marginTop: "20px" }}>
                ...
                {Array.isArray(queryResult) && queryResult.length > 0 ? (
                  ...
                  Object.keys(queryResult[0]).length > 0 ? (
                    ...
                    >
                      PL/SQL executed successfully. No results returned.
                    </p>
                  )
                ) : typeof queryResult === "object" &&
                  (queryResult.status || queryResult.message) ? (
...
END $$;`}
      ...
      {showInsertModal && (
        <InsertRowModal
          ...
          onClose={() => {
            setShowInsertModal(false);
            setInsertModalError(""); // Clear error when closing
          }}
      ...
      {showUpdateModal && (
        <UpdateRowsModal
          ...
          onClose={() => {
            setShowUpdateModal(false);
            setUpdateModalError(""); // Clear error when closing
          }}
