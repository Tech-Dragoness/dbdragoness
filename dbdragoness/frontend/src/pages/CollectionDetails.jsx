...
function CollectionDetails() {
  ...
  const [allTotalDocs, setAllTotalDocs] = useState(0);
  ...
  const fetchCollectionData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setAllTotalDocs(response.data.total_docs || 0);
        setAllTotalPages(response.data.total_pages || 1);
      } else {
  ...
  const handleQuerySubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        try { await addQuery(queryText); } catch (_) {}
        setQueryText("");
        setQueryResult(response.data.result);
        setMessage("Query executed successfully");

        // Soft-refresh the data tab after query execution
        try {
          ...
          if (refreshResponse.data.success) {
            setDocuments(refreshResponse.data.documents || []);
            setAllDocuments(refreshResponse.data.documents || []);
            setTotalDocs(refreshResponse.data.total_docs || 0);
            setTotalPages(refreshResponse.data.total_pages || 1);
            setAllTotalDocs(refreshResponse.data.total_docs || 0);
            setAllTotalPages(refreshResponse.data.total_pages || 1);
          }
        } catch (refreshErr) {
