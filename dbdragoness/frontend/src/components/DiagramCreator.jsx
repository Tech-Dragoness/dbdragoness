  ...
  const loadDiagramData = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // ✅ LOAD EXISTING RELATIONSHIPS (only for SQL/ER diagrams)
        const existingRelationships = data.relationships || [];
        setRelationships(
        ...
        // ✅ AUTO-GENERATE diagram after data loads
        generateMermaidCode(data, diagramType);
      } else {
  ...
  const generateERDiagram = (data) => {
    ...
    tables.forEach((table) => {
      ...
      fields.forEach((field) => {
        ...
        // ✅ Sanitize field type - replace parentheses and commas with underscores
        const sanitizedType = (field.type || "TEXT")
          .replace(/\(/g, "_")     // Replace ( with _
          .replace(/\)/g, "")      // Remove )
          .replace(/,/g, "_")      // Replace , with _
          .replace(/\s+/g, "_")    // Replace spaces with _
          .replace(/[^a-zA-Z0-9_]/g, "") // Remove any other special chars
          .toUpperCase();

        const constraintStr =
          constraints.length > 0 ? ` "${constraints.join(",")}"` : "";
        code += `        ${sanitizedType} ${field.name}${constraintStr}\n`;
      });
      code += "    }\n";
    });
