...
function CreateTableModal({ dbName, onClose, onSuccess }) {
  ...
  const handlePKChange = (index, checked) => {
    ...
    if (checked) {
      ...
      updated[index] = {
        ...updated[index],
        is_pk: false,
        // When unchecking PK, also clear the PK-implied constraints on THIS column only
        is_not_null: false,
        is_unique: false,
      };
    }
  ...
  const handleCreateTable = async (e) => {
    ...
    for (const col of columns) {
      ...
      // MySQL does not allow AUTO_INCREMENT on a composite PK column
      if (isCompositePK && col.is_pk && col.is_autoincrement) {
        setError(
          `Column "${col.name}" cannot be AUTO_INCREMENT when part of a composite primary key.`,
        );
        setLoading(false);
        return;
      }
