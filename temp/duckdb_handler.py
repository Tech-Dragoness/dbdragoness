...
class DuckDBHandler(DBHandler):
    ...
    def get_connection_info(self, db_name):
        """Return DuckDB connection information"""
        return {
            'connection_string': f'duckdb:///sql_dbs/duckdb/{db_name}_duckdb.db',
            'test_code': f'''from sqlalchemy import create_engine, text
import os

# Get absolute path to database file
db_path = os.path.abspath('sql_dbs/duckdb/{db_name}_duckdb.db')
...
if not os.path.exists(db_path):
    ...
    def _get_db_path(self, db_name):
        return os.path.join(self.base_path, f"{db_name}_duckdb.db")
    ...
    def create_db(self, db_name):
        ...
        import glob
        files = glob.glob(os.path.join(self.base_path, "*_duckdb.db"))
        self.logger.info(f"🔍 Files after creation: {files}")
    ...
    def list_dbs(self):
        ...
        for filename in os.listdir(self.base_path):
            if filename.endswith('_duckdb.db'):
                # Extract the database name preserving case from filename
                db_name = filename[:-len('_duckdb.db')]
                dbs.append(db_name)
