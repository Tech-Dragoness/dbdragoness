...
with engine.connect() as conn:
    ...
    def modify_table(self, old_table_name, new_table_name, new_columns):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                temp_table_name = f"temp_{old_table_name}_{os.urandom(4).hex()}"
    ...
    def create_trigger(self, trigger_name: str, table_name: str, trigger_timing: str, trigger_event: str, trigger_body: str):
        ...
        func_name = f"trigger_func_{table_name.lower()}_{trigger_name.lower()}_{os.urandom(4).hex()}"
...
SELECT update_marks(1, 95);"""
    ...
    def add_check_constraint_to_existing_table(self, table_name, column_name, expression, conn=None):
        ...
        try:
            ...
            constraint_name = f"chk_{table_name.lower()}_{column_name}_{os.urandom(4).hex()}"
        ...
        if check_constraints_to_apply:
            ...
            for check_info in check_constraints_to_apply:
                try:
                    ...
                    constraint_name = f"chk_{dest_table}_{check_info['column']}_{os.urandom(2).hex()}"
        ...
        with self.engine.connect() as conn:
            ...
            for row in triggers:
                ...
                try:
                    ...
                    # Generate new unique names (lowercase base to avoid issues)
                    new_func_name = f"trigger_func_{dest_table.lower()}_{os.urandom(4).hex()}"
                    new_trigger_name = f"{dest_table.lower()}_{trigger_name.lower()}_{os.urandom(2).hex()}"
                    ...
                    # Replace old function name in the CREATE FUNCTION header (case-insensitive)
                    new_func_def = re.sub(
                        rf'CREATE\s+FUNCTION\s+([^\s.]+\.)?{re.escape(old_func_name)}\s*\(',
                        f'CREATE FUNCTION {new_func_name}( ',
                        func_def,
                        count=1,
                        flags=re.IGNORECASE
                    )
                    
                    # Replace ALL table references: quoted original, unquoted lowercase, and quoted lowercase
                    ...
                    new_func_def = new_func_def.replace(lower_source, lower_dest)
