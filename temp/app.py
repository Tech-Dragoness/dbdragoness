...
def create_app(initial_db_type, handler_name=None):
    ...
    query_history_manager = QueryHistoryManager(
        history_file=os.path.join(app.root_path, 'data', 'query_history.json')
    )
    ...
    @app.route('/dbdragoness/db/<db_name>/table/<table_name>/chart_data')
    def get_chart_data(db_name, table_name):
        """Generic route to fetch ALL data for charting (bypasses pagination)"""
        try:
    ...
    def serve_react(path='index.html'):
        ...
        if path != 'index.html' and not os.path.exists(os.path.join(react_dir, path)):
            path = 'index.html'
        ...
        try:
            ...
            try:
                ...
                if not keep_old:
                    ...
                    if db_name_type == 'SQLite':
                        # Close and dispose engine completely
                        if handler.engine:
                        ...
                        # Wait longer for Windows to release file handle
                        time.sleep(0.5)
                    else:
                        time.sleep(0.1)
    ...
    @app.route('/api/db/<db_name>/normalize', methods=['POST'])
    def api_normalize_database(db_name):
        """JSON API: Normalize database to specified normal form"""
        try:
            logger.debug(f"✅ API call: POST /api/db/{db_name}/normalize")
            
            app.config['HANDLER'].switch_db(db_name)
            handler = app.config['HANDLER']
            
            if app.config['DB_TYPE'] != 'sql':
                return jsonify({'success': False, 'error': 'Normalization only available for SQL databases'}), 400
            
            data = request.get_json()
            normal_form = data.get('normal_form')
            analysis_data = data.get('analysis_data')
            
            if not normal_form or not analysis_data:
                return jsonify({'success': False, 'error': 'Normal form and analysis data are required'}), 400
            ...
            try:
                ...
                # Get all tables
                all_tables = handler.list_tables()
                
                # Create backup database
                handler.create_db(backup_name)
                
                # Copy all data to backup
                for table in all_tables:
                    handler.switch_db(db_name)
                    schema = handler.get_table_schema(table)
                    data_rows = handler.read(table)
                    
                    handler.switch_db(backup_name)
                    
                    # Build column definitions WITHOUT multiple PKs
                    columns_def = []
                    pk_columns = []
                    ...
                    # Build column definitions
                    for col in schema:
                        col_name = handler._quote_identifier(col['name']) if hasattr(handler, '_quote_identifier') else f'"{col["name"]}"'
                        col_type = col['type']
                        
                        col_def = f"{col_name} {col_type}"
                        
                        # Only mark as PRIMARY KEY if it's the ONLY pk column
                        if col.get('pk') and len(pk_columns) == 1:
                            if col.get('autoincrement'):
                                if hasattr(handler.handler, 'DB_NAME'):
                                    if handler.handler.DB_NAME == 'PostgreSQL':
                                        col_def = f"{col_name} SERIAL PRIMARY KEY"
                                    elif handler.handler.DB_NAME == 'MySQL':
                                        col_def = f"{col_name} {col_type} AUTO_INCREMENT PRIMARY KEY"
                                    else:
                                        col_def += " PRIMARY KEY AUTOINCREMENT"
                                else:
                            ...
                            if col.get('unique') and not col.get('pk'):
                                col_def += " UNIQUE"
                            if col.get('autoincrement') and not col.get('pk'):
                                col_def += " AUTOINCREMENT"
                    ...
                    if len(pk_columns) > 1:
                        pk_clause = f"PRIMARY KEY ({', '.join([handler._quote_identifier(c) if hasattr(handler, '_quote_identifier') else f'\"{c}\"' for c in pk_columns])})"
                        columns_def.append(pk_clause)
                    
                    col_def_str = ', '.join(columns_def)
                    quoted_table = handler._quote_identifier(table) if hasattr(handler, '_quote_identifier') else f'"{table}"'
                ...
                if normal_form in ['1NF', '2NF', '3NF', 'BCNF']:
                    if user_answers.get('repeating_groups') == 'yes':
                        changes.append("Identified repeating groups - requires manual table restructuring")
                
                # 2NF: Remove partial dependencies
                if normal_form in ['2NF', '3NF', '3NF', 'BCNF']:
                    if user_answers.get('partial_dependencies') == 'yes':
                        changes.append("Identified partial dependencies on composite keys - requires splitting tables")
                
                # 3NF: Remove transitive dependencies
                if normal_form in ['3NF', 'BCNF']:
                    if user_answers.get('transitive_dependencies') == 'yes':
                        changes.append("Identified transitive dependencies - requires creating lookup tables")
                
                # BCNF: Ensure all determinants are candidate keys
                if normal_form == 'BCNF':
                    candidate_keys = user_answers.get('candidate_keys', '')
                    if candidate_keys:
                        changes.append(f"Analyzed candidate keys: {candidate_keys}")
                
                return jsonify({
                    'success': True,
                    'message': f'Database analyzed for {normal_form} normalization',
                    'backup_database': backup_name,
                    'changes': changes if changes else [f'Database structure analyzed - no automated changes applied. Backup created for safety.'],
                ...
                try:
                    handler.delete_db(backup_name)
                except:
                    pass
                return jsonify({'success': False, 'error': str(e)}), 500
                
        except Exception as e:
            logger.error(f"❌ API call failed: /api/db/{db_name}/normalize - {e}")
            return jsonify({'success': False, 'error': str(e)}), 500

    @app.route('/api/db/<db_name>/analyze_for_normalization', methods=['GET'])
    def api_analyze_for_normalization(db_name):
        """JSON API: Analyze database structure to suggest normalization"""
        try:
            logger.debug(f"✅ API call: GET /api/db/{db_name}/analyze_for_normalization")
            
            app.config['HANDLER'].switch_db(db_name)
            handler = app.config['HANDLER']
            
            if not hasattr(handler, 'analyze_for_normalization'):
                return jsonify({'success': False, 'error': 'Normalization analysis not supported'}), 400
            
            analysis = handler.analyze_for_normalization()
            ...
            logger.error(f"❌ API call failed: /api/db/{db_name}/analyze_for_normalization - {e}")
            return jsonify({'success': False, 'error': str(e)}), 500
